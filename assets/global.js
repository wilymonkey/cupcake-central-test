...
document.querySelectorAll('[id^="Details-"] summary').forEach((summary) => {
  ...
  if (window.innerWidth >= 990) {
    ...
    var megaMenuContent = detailsElement.querySelector(".mega-menu__content");

    summary.addEventListener("mouseenter", (event) => {
      event.currentTarget.parentNode.setAttribute("open", "");
      event.currentTarget.setAttribute("aria-expanded", true);

      if (megaMenuContent) {
        megaMenuContent.classList.remove("progressive-reveal-in");
        megaMenuContent.classList.remove("progressive-reveal-out");

        void megaMenuContent.offsetWidth;

        setTimeout(() => {
          megaMenuContent.classList.add("progressive-reveal-in");
        }, 2);
      }
    });
    ...
    summary.parentNode.addEventListener("mouseleave", (event) => {
      ...
      if (megaMenuContent) {
        ...
        setTimeout(() => {
          megaMenuContent.classList.add("progressive-reveal-out");
        }, 10);
      }
      // }, 100);
    });
